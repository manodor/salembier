...
class ResPartner(models.Model):
    ...
    def _get_next_ref_supplier(self, vals=None):
        return self.env["ir.sequence"].next_by_code("res.partner.supplier")

    def _get_next_ref_customer(self, vals=None):
    ...
    def create(self, vals):
        if not vals.get("ref") and self._needs_ref(vals=vals):
            ...
            if self._context.get('default_customer_rank'):
                vals["ref"] = self._get_next_ref_customer(vals=vals)
    ...
    def copy(self, default=None):
        ...
        if self._needs_ref():
            if self._context.get('default_supplier_rank'):
                default["ref"] = self._get_next_ref_supplier()
    ...
    def write(self, vals):
        for partner in self:
            ...
            if (
                ...
                if self._context.get('default_customer_rank'):
                    partner_vals["ref"] = partner._get_next_ref_customer(
                        vals=partner_vals
                    )
                if self._context.get('default_supplier_rank'):
                    partner_vals["ref"] = partner._get_next_ref_supplier(
                        vals=partner_vals
                    )
